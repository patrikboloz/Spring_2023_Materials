/* Lab 5
Create these 3 functions and run them in the main()

1) Write a function that converts kilometers per hour to miles per hour
2) Write a function that converts temperature in Kelvin to Fahrenheit
3) Write a function that will compute the total and average of five numbers

EXTRA CREDIT:
EC) Write a function that will accept the radius of a circle from the user and compute the area
   and circumference

Make sure that all functions will ask the user for the parameters and then use those parameters
as arguments in your functions.
*/

